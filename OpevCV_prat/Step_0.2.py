...
def compare(paths, img):
    ...
    ref = cv2.imread(paths[0], cv2.IMREAD_GRAYSCALE)  # Load the first image in grayscale
...
def find_card(img):
    ...
    for card in cards:  # Draw a line through the points to a shape, that has been added above
        cv2.drawContours(copy_image, [card], -1, (0, 255, 0), 2)  # the order is in BGR not RBG for the color
    ...
    if len(cards) != 0:
        try:
            ...
            cv2.putText(copy_image, label, (int(top_left[0]), int(top_left[1] - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)
            cv2.imshow("Detected Card", copy_image)
        except:
            pass
...
def splitt_img(corner):
    ...
    # cv2.imshow("ROI RANK", ROI_RANK)

    ROI_SUIT = corner[((height // 2) + ROI_diff): height, 0:width]
    # cv2.imshow("ROI SUIT", ROI_SUIT)

    # edge = cv2.Canny(ROI_RANK, 50, 150)
    # cv2.imshow("R", edge)
    # edge = cv2.Canny(ROI_SUIT, 50, 150)
    # cv2.imshow("S" ,edge)

    # Make an image to black and white (including gray)
    gray_RANK = cv2.cvtColor(ROI_RANK, cv2.COLOR_BGR2GRAY)
    gray_SUIT = cv2.cvtColor(ROI_SUIT, cv2.COLOR_BGR2GRAY)

    blurred_RANK = cv2.GaussianBlur(gray_RANK, (3, 3), 0)
    blurred_SUIT = cv2.GaussianBlur(gray_SUIT, (3, 3), 0)

    # Highlights the edges in an image
    edge_RANK = cv2.Canny(blurred_RANK, 50, 150)
    edge_SUIT = cv2.Canny(blurred_SUIT, 50, 150)

    # cv2.imshow("gray rank", edge_RANK)
    # cv2.imshow("gray suit", edge_SUIT)

    # Finds the contours in an image (shapes)
    contours_RANK, _ = cv2.findContours(edge_RANK, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours_SUIT, _ = cv2.findContours(edge_SUIT, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # Sort contours by area (largest to smallest) and keep the largest one
    contours_RANK = sorted(contours_RANK, key=cv2.contourArea, reverse=True)[:1]
    contours_SUIT = sorted(contours_SUIT, key=cv2.contourArea, reverse=True)[:1]

    # Extract bounding boxes
    rank_x, rank_y, rank_w, rank_h = cv2.boundingRect(contours_RANK[0])  # Rank
    suit_x, suit_y, suit_w, suit_h = cv2.boundingRect(contours_SUIT[0])  # Suit
    ...
    # cv2.imshow("R I", rank_img)
    # cv2.imshow("S I", suit_img)
