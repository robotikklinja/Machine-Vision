...
def rankandsuit(corner):
    ...
    # Calculate the height of the rank area
    diff_h = round(h * 0.1)  # Adjust based on how much space you want for the rank
    RANK = gray[0:((h // 2) + diff_h), 0:w]  # Crop for Rank area
    SUIT = gray[((h // 2) + diff_h):h, 0:w]  # Crop for Suit area

    # Get the coordinates for the rank bounding box
    rank_x, rank_y, rank_w, rank_h = 0, 0, w, (h // 2) + diff_h

    # Draw a box around the Rank area (without any text inside)
    cv.rectangle(corner, (rank_x, rank_y), (rank_x + rank_w, rank_y + rank_h), (0, 255, 0), 2)  # Green box

    # Optionally, you can display the rank image
    cv.imshow('Rank', RANK)
